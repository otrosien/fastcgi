...
public class FastCGIServlet extends FastCGIProxyServlet {
	...
	protected void customizeFastCGIHeaders(Request proxyRequest,
		...
		try {
			...
			fastCGIHeaders.add("HTTP_X_EPAGES_SITE", shopInfo.getGuid()
					.toString());
		} catch (IOException e) {
		...
		if (proxyRequest.getHeaders().containsKey("Authorization")) {
			fastCGIHeaders.add("HTTP_AUTHORIZATION", proxyRequest.getHeaders()
					.get("Authorization"));
		}
		if (HttpScheme.HTTPS.is((String) proxyRequest.getAttributes().get(
				FastCGIProxyServlet.class.getName() + ".scheme"))) {
